...
const handleToolEvent = (toolData: ToolEventData) => {
  ...
  if (toolContent.name !== 'message') {
    ...
    if (existingIdx >= 0) {
      const merged = { ...activityItems.value[existingIdx].tool } as ToolContent;
...
const handleEvent = (event: AgentSSEEvent) => {
  ...
  const role = event.event === 'message' ? (event.data as MessageEventData).role : undefined;
  const uniqueId = event.event === 'message' ? `${eid}-${role}` : eid;
  if (uniqueId && _processedEventIds.has(uniqueId)) return;
  if (uniqueId) _processedEventIds.add(uniqueId);
...
const restoreSession = async () => {
  ...
  for (const event of session.events) {
    if (isStale()) return;
    const originalEventId = event.data?.event_id;
    if (originalEventId) event.data.event_id = `restore-${event.event}-${originalEventId}`;
